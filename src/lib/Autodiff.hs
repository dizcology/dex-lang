...
data DerivWrt n = DerivWrt { activeVars  :: Env n Type n
                           , _activeEffs :: [Effect n]
                           , rematVars   :: Env n Type n }
-- `Tangents` holds the tangent values and the region variables that are
-- arguments to the linearized function.
data TangentEnv n = TangentEnv { tangentVals :: Subst n n
                               , activeRefs  :: [Name n]
                               , rematVals   :: Subst n n}

-- TODO: use a distinct name space var for the input to the tangent builder?
type PrimalM n = ReaderT (DerivWrt n) (Builder n)
type TangentM n = ReaderT (TangentEnv n) (Builder n)
newtype LinA n a = LinA { runLinA :: PrimalM n (a, TangentM n a) }

linearize :: Scope n -> Atom n -> Atom n
linearize = undefined
...
linearizeBlock :: forall i o. Subst i o -> Block i -> LinA o (Atom o)
...
linearizeExpr :: Subst i o -> Expr i -> LinA o (Atom o)
linearizeExpr env expr = case expr of
  ...
  --       Abs bs' HUnit <- substBuilder env $ Abs bs HUnit
  --       buildNAbs bs' \xs -> tangentFunAsLambda $
  --         linearizeBlock (env <> newEnv (fromNest bs) xs) body
  _ -> LinA $ do
    expr' <- substBuilder env expr
...
linearizeOp :: forall n. Op n -> LinA n (Atom n)
...
linearizeUnOp :: UnOp -> Atom n -> LinA n (Atom n)
...
linearizeBinOp :: BinOp -> Atom n -> Atom n -> LinA n (Atom n)
...
linearizeHof :: forall i o. Subst i o -> Hof i -> LinA o (Atom o)
linearizeHof = undefined
-- linearizeHof env hof = case hof of
--   For ~(RegularFor d) ~(LamVal (i:>iTy) body) -> LinA $ do
--     iTy' <- substBuilder env iTy
--     (ansWithLinTab, vi'') <- buildForAux d iTy' \i''@(Var vi'') ->
--        (,vi'') <$> (willRemat vi'' $ tangentFunAsLambda $ linearizeBlock (env <> i@>i'') body)
--     (ans, linTab) <- unzipTab ansWithLinTab
--     return (ans, buildFor d iTy' \i'' ->
--                    provideRemat vi'' i'' $ app linTab i'' >>= applyLinToTangents)
...
--     bm' <- traverse (substBuilder env) bm
--     ~(RefTy _ accTy) <- substBuilder env $ binderType refBinder
--     linearizeEff env lam Writer (RunWriter bm') (emitRunWriter accTy bm')
--   RunReader val lam -> LinA $ do
--     (val', mkLinInit) <- runLinA <$> linearizeAtom =<< substBuilder env val
--     linearizeEff env lam Reader (RunReader val') \f ->
--       mkLinInit >>= emitRunReader `flip` f
--   RunState val lam -> LinA $ do
--     (val', mkLinInit) <- runLinA <$> linearizeAtom =<< substBuilder env val
--     linearizeEff env lam State (RunState val') \f ->
--       mkLinInit >>= emitRunState `flip` f
  -- RunIO ~(Lam (Abs _ (WithArrow arrow body))) -> LinA $ do
  --   arrow' <- substBuilder env arrow
  --   -- TODO: consider the possibility of other effects here besides IO
  --   lam <- buildLam UnitTy arrow' \_ ->
...
linearizeEff :: Subst i o -> Atom i -> RWS
             -> (Atom o -> Hof o)
             -> ((Atom o -> TangentM o (Atom o)) -> TangentM o (Atom o))
             -> PrimalM o (Atom o, TangentM o (Atom o))
linearizeEff = undefined
-- linearizeEff env lam eff primalHofCon tangentEmitter = do
--   (lam', ref) <- linearizeEffectFun env eff lam
...
linearizePrimCon :: Con n -> LinA n (Atom n)
...
linearizeAtom :: Atom n -> LinA n (Atom n)
linearizeAtom atom = case atom of
  ...
  DataCon _ _ _   -> notImplemented  -- Need to synthesize or look up a tangent ADT
  ...
  Lam _ _         -> error "Unexpected non-table lambda"
...
withZeroTangent :: Atom n -> (Atom n, TangentM n (Atom n))
...
tangentFunAsLambda :: forall n. LinA n (Atom n) -> PrimalM n (Atom n)
...
applyLinToTangents :: Atom n -> TangentM n (Atom n)
...
bindLin :: LinA n a -> (a -> Builder n b) -> LinA n b
bindLin (LinA m) f = LinA $ do
  (e, t) <- m
  x <- lift $ f e
  return (x, t >>= (lift . f))

instance Functor (LinA n) where
  fmap = liftA

instance Applicative (LinA n) where
...
isActive :: Name n -> PrimalM n Bool
...
willRemat :: Name n -> PrimalM n a -> PrimalM n a
willRemat = undefined
...
provideRemat :: Name n -> Atom n -> TangentM n a -> TangentM n a
