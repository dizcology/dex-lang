...
parallelizeModule :: Module i -> Module o
parallelizeModule = undefined
...
-- data ABlock n = ABlock (Nest Decl n) (Atom n)

-- asABlock :: Block n -> ABlock n
...
-- data LoopEnv i o = LoopEnv
--   { loopBinders :: [Var i]
--   , delayedApps :: Env i (Atom o, [Atom i])
--   }
-- data AccEnv n = AccEnv { activeAccs :: Env n (Var n, BaseMonoid n) } -- (reference, its base monoid)

-- type TLParallelM n = SubstBuilderT i o (State (AccEnv n))   -- Top-level non-parallel statements
-- type LoopM i o   = ReaderT (LoopEnv i o) TLParallelM  -- Generation of (parallel) loop nests
