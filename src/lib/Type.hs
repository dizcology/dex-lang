...
module Type (
  getType, checkType, HasType (..), litType,
  isPure, functionEffs, exprEffs, blockEffs, extendEffect, isData, checkBinOp, checkUnOp,
  checkIntBaseType, checkFloatBaseType, checkExtends, indexSetConcreteSize,
  projectLength, typeReduceBlock, typeReduceAtom,
  typeReduceExpr, oneEffect) where
...
-- TODO: use a type class to switch between querying and checking? Might be
-- faster because it's more static.
data TypeCheckEnv n = TypeCheckEnv

type TypeM n = ReaderT (TypeCheckEnv n) Except

class (HasVars e, PrettyH e) => HasType e where
  typeCheck :: Pretty (e n) => e n -> TypeM n (Type n)

getType :: (HasCallStack, HasType e) => Bindings n -> e n -> Type n
getType = undefined
-- getType x = ignoreExcept $ ctx $ runTypeCheck SkipChecks $ typeCheck x
--   where ctx = addContext $ "Querying:\n" ++ pprint x

checkType :: HasType e => Bindings n -> EffectRow n -> e n -> Except ()
checkType = undefined
-- checkType env eff x = void $ ctx $ runTypeCheck (CheckWith (env, eff)) $ typeCheck x
--   where ctx = addContext $ "Checking:\n" ++ pprint x

runTypeCheck :: TypeCheckEnv n -> TypeM n a -> Except a
runTypeCheck env m = runReaderT m env

lookupType :: Name n -> TypeM n (Type n)
lookupType = undefined

-- -- For debugging
-- traceCheckM :: (HasCallStack, HasVars e, HasType e, Monad m)
--             => e n -> m ()
-- traceCheckM x = traceCheck x (return ())

-- traceCheck :: (HasCallStack, HasVars e, HasType e)
--            => e n -> a -> a
-- traceCheck x y =
...
-- checkDataConRefBindings :: Nest Binder n -> Nest DataConRefBinding n -> TypeM n ()
...
--   checkEq (binderType b) (binderType b')
...
checkCase :: (Pretty (b n), HasType b)
          => Atom n -> HList (AltP b) n -> Type n -> TypeM n (Type n)
checkCase = undefined
...
checkApp :: Type n -> Atom n -> TypeM n (Type n)
checkApp = undefined
-- checkApp fTy x = do
--   Pi piTy <- return fTy
--   x |: absBinding piTy
...
checkDecl :: Bindings n -> Decl n -> TypeM n (Bindings n)
checkDecl = undefined
...
checkArrow :: Arrow n -> TypeM n ()
checkArrow = undefined -- mapMH_ checkEffRow

infixr 7 |:
(|:) :: HasType e => e n -> Type n -> TypeM n ()
(|:) x reqTy = do
  ty <- typeCheck x
  checkEq reqTy ty

checkEq :: (Pretty a, Eq a) => a -> a -> TypeM n ()
...
-- checkNoShadow :: (MonadError Err m) => Env n a -> Binder n -> m ()
...
instance CoreVariant (Binding n) where
  -- let annotation restrictions?
  checkVariant (Let ty _ expr) = checkVariant ty >> checkVariant expr
...
checkEffRow :: EffectRow n -> TypeM n ()
...
declareEff :: Effect n -> TypeM n ()
declareEff eff = declareEffs $ oneEffect eff

declareEffs :: EffectRow n -> TypeM n ()
...
checkLabeledRow :: ExtLabeledItems (Type n) (Name n) -> TypeM n ()
...
labeledRowDifference :: ExtLabeledItems (Type n) (Name n)
                     -> ExtLabeledItems (Type n) (Name n)
                     -> TypeM n (ExtLabeledItems (Type n) (Name n))
...
checkWithEnv :: (EffectRow n -> TypeM n ()) -> TypeM n ()
checkWithEnv = undefined
...
updateTypeEnv :: (Bindings n -> Bindings n) -> TypeM n a -> TypeM n a
updateTypeEnv = undefined
-- updateTypeEnv f m = flip local m $ fmap \(env, eff) -> (f env, eff)

extendTypeEnv :: Bindings n -> TypeM n a -> TypeM n a
extendTypeEnv new m = undefined -- updateTypeEnv (<> new) m

withTypeEnv :: Bindings n -> TypeM n a -> TypeM n a
withTypeEnv new m = updateTypeEnv (const new) m

extendAllowedEffect :: Effect n -> TypeM n () -> TypeM n ()
extendAllowedEffect eff m = updateAllowedEff (extendEffect eff) m

updateAllowedEff :: (EffectRow n -> EffectRow n) -> TypeM n a -> TypeM n a
updateAllowedEff = undefined
-- updateAllowedEff f m = flip local m $ fmap \(env, eff) -> (env, f eff)

withAllowedEff :: EffectRow n -> TypeM n a -> TypeM n a
withAllowedEff eff m = updateAllowedEff (const eff) m

-- === primitive ops and constructors ===

typeCheckTyCon :: TC n -> TypeM n (Type n)
...
typeCheckCon :: Con n -> TypeM n (Type n)
...
typeCheckRef :: Atom n -> TypeM n (Type n)
typeCheckRef x = do
...
checkValidCast :: Type n -> Type n -> TypeM n ()
...
typeCheckOp :: Op n -> TypeM n (Type n)
typeCheckOp op = case op of
  ...
  PrimEffect ref m -> do
    TC (RefType ~(Just (Var h')) s) <- typeCheck ref
    case m of
...
typeCheckHof :: Hof n -> TypeM n (Type n)
typeCheckHof hof = case hof of
  ...
  While body -> do
    Pi (NoAbs HUnit arr) (NoAbs UnitTy condTy) <- typeCheck body
    declareEffs $ arrowEff arr
...
checkRWSAction :: RWS -> Atom n -> TypeM n (Type n, Type n)
...
-- checkDataLikeDataCon :: MonadError Err m => DataConDef n -> m ()
