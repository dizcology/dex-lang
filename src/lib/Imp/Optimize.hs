...
type ModAllocEnv  = SimpleEnv () [AllocInfo]

liftCUDAAllocations :: ImpModule -> ImpModule
liftCUDAAllocations = undefined
...
--                   newBody <- extendValSubst (newEnv (map fst argBs') $ fmap IVar args) $ buildScoped $ do
...
--           extend $ fname @> allocs
--           let newFunTy = IFunType cc (argTys ++ fmap snd allocBs) retTys
...
--         _ -> traverseImpFunction amendLaunch fenv f
    ...
    --           Heap GPU -> do
    --             bArg <- freshIVar b
    --             liftSE $ extend $ [(bArg, (ty, fromIntegral size))]
